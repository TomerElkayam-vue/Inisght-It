...
const dataTypeToText: Record<GithubDataType, string> = {
  [GithubDataType.COMMENTS]: 'Comments',
  [GithubDataType.COMMITS]: 'Commits',
  [GithubDataType.FILE_CHANGES]: 'File Changes',
  [GithubDataType.PR]: 'Pull Request',
};

export const CodeRepositoryDashboard = () => {
  const [toggle, setToggle] = useState('user');
  const [selectedDataType, setSelectedDataType] = useState<GithubDataType>(
    GithubDataType.PR
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        setIsLoading(true);
        const statsResponse = await getGithubStats(
          currentProject.id,
          selectedDataType,
          toggle === 'team'
        );
